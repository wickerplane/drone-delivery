...
;    is
;        K(r(1+r)^n)/((1+r)^n - 1))
    report 100  ; this is a dummy value. Replace it properly after some code.
end

to-report hoursperyear [hoursperday]
  ; Returns hours per year given hours per day.
  ; Assumes 365.25 days per year.
  report 100  ; this is a dummy value. Replace it properly after some code.
end

to-report annualusagecost [hoursperday watts centsakwhour]
  ; Returns annual cost of electric power usage in
  ; dollars, where consumption given in watt and
...
to-report a-b-payback-period
  ; if the A-price is not larger than the B-price,
  ; report "A must cost more than B for this analysis."
  ; Otherwise, report the payback period in years for
  ; the a-price - b-price differential. That is, this value
  ; divided by (the b annual cost minus the a annual cost).
  report 100  ; this is a dummy value. Replace it properly after some code.
end
@#$#@#$#@
GRAPHICS-WINDOW
...
532
201
A annualized capital cost in $
AgivenP A-price hurdle-rate A-life-in-years
4
1
11
...
514
246
A annual  usage cost in $
annualusagecost hours-per-day A-watts centsperkwhour
4
1
11
...
702
201
B annualized capital cost in $
AgivenP B-price hurdle-rate B-life-in-years
4
1
11
...
702
246
B annual usage cost in $
annualusagecost hours-per-day B-watts centsperkwhour
4
1
11
...
514
291
A levelized annual cost in $
annualusagecost hours-per-day A-watts centsperkwhour + AgivenP A-price hurdle-rate A-life-in-years
4
1
11
...
702
291
B levelized annual cost in $
AgivenP B-price hurdle-rate B-life-in-years + annualusagecost hours-per-day B-watts centsperkwhour
4
1
11
...
519
42
Simple Levelized Lighting Calculator 
18
0.0
1
...
Item-Type
Item-Type
"Greenlite LED" "GE Soft White" "GE Halogen" "Standard 60 W incandescent"
2
...
A simple calculator for comparing the levelized costs of two energy services investments.
See http://www.ase.tufts.edu/gdae/education_materials/modules/RenewableEnergyEcon.pdf for a good, but dated (renewables are much cheaper now) overview of renewable electric power economics.
